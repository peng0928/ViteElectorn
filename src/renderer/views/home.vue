<template>
  <div class="pb-10">
    <div>
      <a-carousel autoplay>
        <div v-for="item in headShop" class="rounded-2xl h-64 w-full ">
          <div class="flex justify-center items-center rounded-2xl h-full">
            <img
                :src="item.imgUrl"
                class="w-[80%] h-full rounded-2xl transition-transform hover:scale-105 duration-500"
                alt="演出海报"
            >
          </div>
        </div>
    ...
    <div>
      <div @mousemove="handleMouseMove"
           ...
           ref="scrollContainer" @wheel.prevent="handleWheel"
           class="p-5 flex gap-5 w-full overflow-x-auto scrollbar-hide">
        <div
            class="w-64 rounded-b-2xl transform duration-500 hover:-translate-y-2 shadow-xl" v-for="item in hotShop"
            :key="item.id">
          <div
              class="lg:w-45 sm:w-32  flex-shrink-0 hover:ring-3 hover:ring-black  rounded-2xl">
