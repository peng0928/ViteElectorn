<template>
    <div class="p-5 pb-15">
        ...
        <!-- 订单列表 -->
        <div
            class="p-5 grid gap-12 2xl:grid-cols-3 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-1 sm:grid-cols-1 xs:grid-cols-1 h-[calc(100vh-280px)] overflow-y-auto scrollbar-hide rounded-2xl">
            <div v-for="order in paginatedOrders" :key="order.id" :class="[
                'relative h-64 w-96 p-4 rounded-lg transition-all duration-300',
...
const getOrders = async () => {
    try {
        const response = await axios.post('/api/orders', { pageNum: currentPage.value, pageSize: pageSize.value, type: orderType.value });
